...
export default function App() {
  ...
  // ── Auto-load from Design Session transfer ──
  useEffect(() => {
    const designSession = searchParams.get('design_session');
    if (!designSession) return;

    const loadSessionData = async () => {
      let data = null;
      const raw = sessionStorage.getItem('design_transfer');
      if (raw) {
        try {
          data = JSON.parse(raw);
          sessionStorage.removeItem('design_transfer');
        } catch (e) {
          console.error('Design transfer parse error:', e);
        }
      }

      // If design_transfer was not in sessionStorage (e.g. page refreshed), fetch from server
      if (!data) {
        try {
          const resp = await fetch(`/api/design/sessions/${designSession}/send-to-quoting`, { method: 'POST' });
          if (resp.ok) {
            data = await resp.json();
          }
        } catch (err) {
          console.error('Failed to fetch design session from server:', err);
        }
      }
      ...
      // Build metrics object matching the expected format
      const metricsObj = {
        volume: m.volume,
        ...
        sizeY: m.sizeY,
        sizeZ: m.sizeZ,
        fileName: `AccuDesign_${data.component_type}_${data.session_id}.step`,
        material: m.material,
        designSession: data.session_id,
        source: 'step', // Ensure it treats it like a STEP file
      };
      setMetrics(metricsObj);

      const geom = buildGeometry(metricsObj);
      setGeometry(geom);

      let cadFile = null;
      if (data.download_url) {
        try {
          // Fetch the CAD file generated by backend
          const resp = await fetch(data.download_url);
          if (resp.ok) {
            const blob = await resp.blob();
            cadFile = new File([blob], metricsObj.fileName, { type: 'model/step' });
          }
        } catch (err) {
          console.error('Failed to download auto-generated CAD:', err);
        }
      }
      ...
      setParts([{
        ...
        metrics: metricsObj,
        geometry: geom,
        brepStatus: cadFile ? 'loading' : 'ready', // Viewer will load the file if present
        fileName: metricsObj.fileName,
        engineeringData: data.engineering_data,
        file: cadFile, // Attached here so Viewer.jsx automatically parses it!
      }]);
      setActivePartId(newId);
      setBrepStatus(cadFile ? 'loading' : 'ready');

      setChatMessages([{
        role: 'assistant',
        text: `✅ Design session ${data.session_id} (${data.component_type.toUpperCase()}) has been automatically transferred.\n\nGeometry loaded — Volume: ${m.volume.toFixed(1)} mm³, Material: ${m.material}.\nEngineering calculations and safety data are attached. Ready for quoting.`
      }]);
    };
  ...
  const handleMetrics = async (m, file, result) => {
    ...
    // Preserve existing engineering data if we are reloading the same auto-generated file
    let existingEngData = null;
    let existingChat = null;
    if (file && parts.length > 0 && parts[0].fileName === (m.fileName || file.name)) {
      existingEngData = parts[0].engineeringData;
      // Preserve chat if it starts with the checkmark (our auto-load message)
      if (chatMessages.length > 0 && chatMessages[0].text.startsWith('✅')) {
    ...
    const newPart = {
      ...
      geometry: geom,
      file: file,
      fileName: m.fileName || file?.name || 'Part 1',
      occtResult: result,
      ...
      aiValidation: null,
      engineeringData: existingEngData, // Preserve the design session data!
    };
    setParts([newPart]);
    setActivePartId(newId);
    ...
    // Preserve chat message for auto-loaded CAD files too
    if (existingChat) {
      setChatMessages(existingChat);
    }
